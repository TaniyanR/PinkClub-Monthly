...
FANZAの月額動画チャンネルに対応したアフィリエイト商品サイトです。[PinkClub-FANZA](https://github.com/TaniyanR/PinkClub-FANZA) を基盤に、対象商品とAPI取得先を分離しています。
...
複数チャンネルをcron実行ごとに巡回し、取得位置をチャンネル別に保持します。
...
| --- | --- | --- | --- |
| FANZA | monthly | monthly_videoa | 見放題ch |
| FANZA | monthly | monthly_videoc | 見放題ch デラックス |
| FANZA | monthly | monthly_vr | VRch |
...
```bash
php /path/to/PinkClub-Monthly/scripts/auto_import.php
```
